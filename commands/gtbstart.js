...
module.exports = {
    ...
    execute(message) {
        ...
        if (gameStatus.includes(message.guild.id)) {
            message.delete();
            return message.channel.send(`[⚠️] There is already a current **Guess The Blank** game being hosted in <#${gameChannel}>.`).then(m => m.delete({
                timeout: 5000
            }))
        }
            ...
            try {
                ...
                setTimeout(function() {
                    ...
                    message.channel.awaitMessages(LV1, {
                        ...
                        .then(collected => {
                            if (collected.first().author.id !== collected.last().author.id) {
