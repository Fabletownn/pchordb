...
module.exports = {
    ...
    execute(message) {
        ...
        channel.messages.fetch(messageID).then(toEdit => {
            ...
            toEdit.edit(editContent).then(message.channel.send(`**[📝] ${message.author.username}**, the message has successfully been edited with the provided content(s) below: (embeds are not impacted).\n\`\`\`${editContent}\`\`\``));
        }).catch(error => {
